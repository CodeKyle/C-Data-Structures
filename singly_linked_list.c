/*  singly_linked_list.c
    Implementation of a singly-linked list. The first element of a list
    contains a sentinel value that cannot be changed or deleted until 
    program termination. This causes additional memory to be used but allows me to easily insert and remove nodes since there is always guaranteed to be at least one element present. 
...
/*  Creates an empty list. Returns a pointer to a node element whose 'value'
    is equal to 'SENTINEL' and whose 'next' pointer is equal to NULL. 'SENTINEL' 
    is used as a sentinel value and is treated as if it were a NULL element. */
struct node* create_list()
{
    ...
    *new_list = (struct node){SENTINEL, NULL};
    return new_list;
}
...
/*  Inserts an entry at the beginning of the list denoted by 'list_head'.
    ...
    node with 'value' of 'SENTINEL', though this will never be displayed. This 
...
bool insert_entry(struct node *list_head, int value)
{
    ...
    *new_entry = (struct node){value, list_head->next};
...
/*  Removes specified 'entry' in list pointed to by 'list_head'. Due to the
    singly implementation, this function runs in O(n) time because it must
    traverse the from the head to find the node before 'entry'. */
bool remove_entry(struct node *list_head, struct node *entry)
{
...
int main(void)
{
    ...
    while (value >= 0)
    {
        ...
        if ( (scanf("%i", &value) == 1) && value >= 0)
        {
            ...
            if (entry != NULL)
            {
                if (!remove_entry(list_head, entry))
